                ...
                <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-amber-50 sticky top-0 z-10">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-black text-xl text-gray-900">Create Notification</h3>
                        <p className="text-sm text-gray-600">Send a notification to users</p>
                      </div>
                    </div>
                    <motion.button
                      onClick={() => setShowCreateModal(false)}
            ...
            <div className="fixed inset-0 z-[101] flex items-center justify-center p-4">
              ...
              >
                <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-amber-50">
                  ...
                  <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-br ${getTypeBgColor(selectedNotification.type)}`}>
                    {getTypeIcon(selectedNotification.type)}
                    <span className="font-bold text-gray-700 uppercase text-sm">{selectedNotification.type}</span>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-4 overflow-y-auto max-h-[60vh]">
                  {selectedNotification.imageUrl && (
                    <div className="w-full h-48 rounded-xl overflow-hidden mb-4 shadow-md">
                      <img src={selectedNotification.imageUrl} alt="" className="w-full h-full object-cover" />
                    </div>
                  )}

                  <div>
                    <label className="block text-sm font-bold text-gray-500 mb-1">Title</label>
                    <p className="font-bold text-gray-900 text-lg">{selectedNotification.title}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-700">{new Date(selectedNotification.timestamp).toLocaleString()}</p>
                  </div>
                  {selectedNotification.actionUrl && (
                    <div>
                      <label className="block text-sm font-bold text-gray-500 mb-1">Action URL</label>
                      <a href={selectedNotification.actionUrl} target="_blank" rel="noopener noreferrer" className="text-orange-500 hover:underline break-all">
                        {selectedNotification.actionUrl}
                      </a>
                    </div>
                  )}
                  {selectedNotification.productId && (
                     <div>
                       ...
                       <p className="text-gray-700 font-medium flex items-center gap-2">
                         ...
                         {products.find(p => p.id === selectedNotification.productId)?.name || selectedNotification.productId}
                       </p>
                     </div>
                  )}
                  {selectedNotification.dealId && (
                     <div>
                       ...
                       <p className="text-gray-700 font-medium flex items-center gap-2">
                         ...
                         {deals.find(d => d.id === selectedNotification.dealId)?.title || selectedNotification.dealId}
                       </p>
                     </div>
                  )}
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-700">
                      {selectedNotification.targetUserId === 'all' ? 'All Users' : `User ID: ${selectedNotification.targetUserId}`}
                    </p>
                  </div>
                </div>
              </motion.div>
